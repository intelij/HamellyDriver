...
fileprivate extension ACFloatingTextfield {
    ...
    func addErrorPlaceholderLabel() -> Void {
        ...
        let frameError = labelErrorPlaceholder?.frame;
        var frameErr = frameError
        frameErr?.size.height = 15;
        frameErr?.origin.y = self.bounds.size.height - (frameError?.size.height ?? 0)!;
        frameErr?.origin.x = 0;
    ...
    func hideErrorPlaceHolder(){
        ...
        var labframe = labelErrorFrame
        labframe?.origin.y = (labelErrorFrame?.origin.y ?? 0)! - 6;
        UIView.animate(withDuration: 0.2) {
